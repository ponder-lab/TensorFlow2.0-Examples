...
class PNet(tf.keras.Model):
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=(1, 12, 12, 3), dtype=tf.float32)])
...
class RNet(tf.keras.Model):
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=(1, 24, 24, 3), dtype=tf.float32)])
    def call(self, x, training=False):
        out = self.prelu1(self.conv1(x))
...
class ONet(tf.keras.Model):
    ...
    @tf.function(input_signature=[tf.TensorSpec(shape=(1, 48, 48, 3), dtype=tf.float32)])
    def call(self, x, training=False):
        out = self.prelu1(self.conv1(x))
        out = tf.nn.max_pool2d(out, 3, 2, padding="SAME")
        out = self.prelu2(self.conv2(out))
